...
// A helper object for signing messages from Primenodes
CDarkSendSigner darkSendSigner;
// The current darksends in progress on the network
std::vector<CDarksendQueue> vecDarksendQueue;
// Keep track of the used Primenodes
std::vector<CTxIn> vecPrimenodesUsed;
// keep track of the scanning errors I've seen
map<uint256, CDarksendBroadcastTx> mapDarksendBroadcastTxes;
// Keep track of the active Primenode
CActivePrimenode activePrimenode;

// count peers we've requested the list from
int RequestedPrimeNodeList = 0;
...
{
    if(fLiteMode) return; //disable all darksend/Primenode related functionality
    ...
    if (strCommand == "dsa") { //DarkSend Accept Into Pool
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PRIMENODE_REJECTED, strError);

            return;
        }

        if(!fPrimeNode){
            std::string strError = _("This is not a Primenode.");
            LogPrintf("dsa -- not a Primenode! \n");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PRIMENODE_REJECTED, strError);
        ...
        std::string error = "";
        CPrimenode* pmn = mnodeman.Find(activePrimenode.vin);
        if(pmn == NULL)
        {
            std::string strError = _("Not in the Primenode list.");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PRIMENODE_REJECTED, strError);
            return;
        }
        ...
        if(sessionUsers == 0) {
            if(pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountPrimenodesAboveProtocol(MIN_POOL_PEER_PROTO_VERSION)/5 > mnodeman.nDsqCount){
        ...
        if(!IsCompatibleWithSession(nDenom, txCollateral, error))
        {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PRIMENODE_REJECTED, error);
            return;
        } else {
            LogPrintf("dsa -- is compatible, please submit! \n");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PRIMENODE_ACCEPTED, error);
            return;
        }
        ...
        CPrimenode* pmn = mnodeman.Find(dsq.vin);
        if(pmn == NULL) return;

        // if the queue is ready, submit if we can
        if(dsq.ready) {
            if(!pSubmittedToPrimenode) return;
            if((CNetAddr)pSubmittedToPrimenode->addr != (CNetAddr)addr){
                LogPrintf("dsq - message doesn't match current Primenode - %s != %s\n", pSubmittedToPrimenode->addr.ToString().c_str(), addr.ToString().c_str());
                return;
            }
            ...
            if(pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountPrimenodesAboveProtocol(MIN_POOL_PEER_PROTO_VERSION)/5 > mnodeman.nDsqCount){
                LogPrint("darksend", "dsq -- Primenode sending too many dsq messages. %s \n", pmn->addr.ToString().c_str());
                return;
            }
            mnodeman.nDsqCount++;
        ...
        if(!fPrimeNode){
            LogPrintf("dsi -- not a Primenode! \n");
            error = _("This is not a Primenode.");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PRIMENODE_REJECTED, error);
        ...
        if(AddEntry(in, nAmount, txCollateral, out, error)){
            ...
            RelayStatus(sessionID, GetState(), GetEntriesCount(), PRIMENODE_RESET);
        } else {
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), PRIMENODE_REJECTED, error);
        }
        ...
        if(!pSubmittedToPrimenode) return;
        if((CNetAddr)pSubmittedToPrimenode->addr != (CNetAddr)pfrom->addr){
            //LogPrintf("dssu - message doesn't match current Primenode - %s != %s\n", pSubmittedToPrimenode->addr.ToString().c_str(), pfrom->addr.ToString().c_str());
            return;
        }
        ...
        if(success){
        darkSendPool.Check();
            RelayStatus(darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), PRIMENODE_RESET);
        }
        ...
        if(!pSubmittedToPrimenode) return;
        if((CNetAddr)pSubmittedToPrimenode->addr != (CNetAddr)pfrom->addr){
            //LogPrintf("dsc - message doesn't match current Primenode - %s != %s\n", pSubmittedToPrimenode->addr.ToString().c_str(), pfrom->addr.ToString().c_str());
            return;
        }
        int sessionIDMessage;
        CTransaction txNew;
    ...
    switch(state) {
        ...
        case POOL_STATUS_ACCEPTING_ENTRIES:
            if(entriesCount == 0) {
                ...
                std::string suffix = "";
                if(     showingDarkSendMessage % 70 <= 40) return strprintf(_("Submitted following entries to primenode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                ...
                return strprintf(_("Submitted to primenode, waiting for more entries ( %u / %d ) %s"), entriesCount, GetMaxPoolTransactions(), suffix);
            }
        ...
        case POOL_STATUS_QUEUE:
            ...
            return strprintf(_("Submitted to primenode, waiting in queue %s"), suffix);;
       default:
...
//
// Check the Darksend progress and send client updates if a Primenode
//
void CDarksendPool::Check()
{
    if(fPrimeNode) LogPrint("darksend", "CDarksendPool::Check() - entries count %lu\n", entries.size());
    ...
    if(fPrimeNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
    ...
    if((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis()-lastTimeChanged >= 10000) {
        ...
        UnlockCoins();
        SetNull();
        if(fPrimeNode) RelayStatus(sessionID, GetState(), GetEntriesCount(), PRIMENODE_RESET);
    }
}

void CDarksendPool::CheckFinalTransaction()
{
    if (!fPrimeNode) return; // check and relay final tx only on primenode
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        if(!darkSendSigner.SetKey(strPrimeNodePrivKey, strError, key2, pubkey2))
        {
            LogPrintf("CDarksendPool::Check() - ERROR: Invalid Primenodeprivkey: '%s'\n", strError);
            return;
        }
...
//
// How does this work? Messages to Primenodes come in via "dsi", these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Primenode
    ...
    if(fPrimeNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Parlay and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002PAR per transaction on average.
            */
            if(r <= 10)
            {
...
void CDarksendPool::CheckTimeout(){
    if(!fEnableDarksend && !fPrimeNode) return;
    ...
    int addLagTime = 0;
    if(!fPrimeNode) addLagTime = 10000; //if we're the client, give the server a few extra seconds before resetting.
...
//
// Execute a darksend denomination via a Primenode.
// This is only ran from clients
//
void CDarksendPool::SendDarksendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount){

    if(fPrimeNode) {
        LogPrintf("CDarksendPool::SendDarksendDenominate() - Darksend from a Primenode is not supported currently.\n");
        return;
    }
    ...
    // we should already be connected to a Primenode
    if(!sessionFoundPrimenode){
        LogPrintf("CDarksendPool::SendDarksendDenominate() - No Primenode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
...
// Incoming message from Primenode updating the progress of darksend
...
bool CDarksendPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, std::string& error, int newSessionID){
    ...
    if(error.size() > 0) strAutoDenomResult = _("Primenode:") + " " + error;
    ...
    if(newState == POOL_STATUS_ACCEPTING_ENTRIES){
        if(newAccepted == 1){
            ...
            sessionFoundPrimenode = true;
            //wait for other users. Primenode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundPrimenode) {
            LogPrintf("CDarksendPool::StatusUpdate - entry not accepted by Primenode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
            DoAutomaticDenominating(); //try another Primenode
        }
        if(sessionFoundPrimenode) return true;
    }
...
//
// After we receive the finalized transaction from the Primenode, we must
...
bool CDarksendPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node){
   ...
   // push all of our signatures to the Primenode
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if(mnodeman.size() == 0){
        LogPrint("darksend", "CDarksendPool::DoAutomaticDenominating - No Primenodes detected\n");
        strAutoDenomResult = _("No Primenodes detected.");
        return false;
    }
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeParlayAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    // initial phase, find a Primenode
    if(!sessionFoundPrimenode){
        ...
        //if we've used 90% of the Primenode list then drop all the oldest first
        int nThreshold = (int)(mnodeman.CountEnabled(MIN_POOL_PEER_PROTO_VERSION) * 0.9);
        LogPrint("darksend", "Checking vecPrimenodesUsed size %d threshold %d\n", (int)vecPrimenodesUsed.size(), nThreshold);
        while((int)vecPrimenodesUsed.size() > nThreshold){
            vecPrimenodesUsed.erase(vecPrimenodesUsed.begin());
            LogPrint("darksend", "  vecPrimenodesUsed size %d threshold %d\n", (int)vecPrimenodesUsed.size(), nThreshold);
        }
        ...
        if(nUseQueue > 33){
            ...
            BOOST_FOREACH(CDarksendQueue& dsq, vecDarksendQueue){
                ...
                bool fUsed = false;
                //don't reuse Primenodes
                BOOST_FOREACH(CTxIn usedVin, vecPrimenodesUsed){
                    if(dsq.vin == usedVin) {
                        fUsed = true;
                        break;
                    }
                ...
                // connect to Primenode and submit the queue request
                ...
                if(pnode != NULL)
                {
                    ...
                    if(pmn == NULL)
                    {
                            ...
                            LogPrintf("DoAutomaticDenominating --- dsq vin %s is not in Primenode list!", dsq.vin.ToString());
                            continue;
                    }
                    pSubmittedToPrimenode = pmn;
                    vecPrimenodesUsed.push_back(dsq.vin);
                    ...
                    strAutoDenomResult = _("Error connecting to Primenode.");
                    dsq.time = 0; //remove node
                    continue;
                }
            }
        ...
        while(i < 10)
        {
            CPrimenode* pmn = mnodeman.FindRandomNotInVec(vecPrimenodesUsed, MIN_POOL_PEER_PROTO_VERSION);
            if(pmn == NULL)
            {
                LogPrintf("DoAutomaticDenominating --- Can't find random primenode!\n");
                strAutoDenomResult = _("Can't find random Primenode.");
                return false;
            }
            ...
            lastTimeChanged = GetTimeMillis();
            LogPrintf("DoAutomaticDenominating -- attempt %d connection to Primenode %s\n", i, pmn->addr.ToString().c_str());
            CNode* pnode = ConnectNode((CAddress)pmn->addr, NULL, true);
            if(pnode != NULL){
                pSubmittedToPrimenode = pmn;
                vecPrimenodesUsed.push_back(pmn->vin);
                ...
                vecPrimenodesUsed.push_back(pmn->vin); // postpone MN we wasn't able to connect to
                i++;
                continue;
            }
        }

        strAutoDenomResult = _("No compatible Primenode found.");
        return false;
    }
...
bool CDarksendPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral,  std::string& strReason)
{
    ...
    if((state != POOL_STATUS_ACCEPTING_ENTRIES && state != POOL_STATUS_QUEUE) || sessionUsers >= GetMaxPoolTransactions()){
        ...
        if(sessionUsers >= GetMaxPoolTransactions()) strReason = _("Primenode queue is full.");
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100PAR+1 ( bit on if present )
    // bit 1 - 10PAR+1
    // bit 2 - 1PAR+1
    // bit 3 - .1PAR+1
    // bit 3 - non-denom
...
std::string CDarksendPool::GetMessageByID(int messageID) {
    ...
    case ERR_MN_LIST: return _("Not in the Primenode list.");
    case ERR_MODE: return _("Incompatible mode.");
    case ERR_NON_STANDARD_PUBKEY: return _("Non-standard public key detected.");
    case ERR_NOT_A_MN: return _("This is not a Primenode.");
    case ERR_QUEUE_FULL: return _("Primenode queue is full.");
...
bool CDarksendQueue::Sign()
{
    ...
    if(!darkSendSigner.SetKey(strPrimeNodePrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CDarksendQueue():Relay - ERROR: Invalid Primenodeprivkey: '%s'\n", errorMessage);
        return false;
    }
...
bool CDarksendQueue::CheckSignature()
{
    ...
    if(pmn != NULL)
    {
        ...
        if(!darkSendSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage, errorMessage)){
            return error("CDarksendQueue::CheckSignature() - Got bad Primenode address signature %s \n", vin.ToString().c_str());
        }
...
void CDarksendPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    ...
    CNode* pnode = FindNode(pSubmittedToPrimenode->addr);
    if(pnode != NULL) {
...
void ThreadCheckDarkSendPool()
{
    ...
    while (true)
    {
        ...
        if(darkSendPool.IsBlockchainSynced()) {
            ...
            if(c % PRIMENODE_PING_SECONDS == 1) activePrimenode.ManageStatus();

            if(c % 60 == 0)
            {
                mnodeman.CheckAndRemove();
                mnodeman.ProcessPrimenodeConnections();
                primenodePayments.CleanPaymentList();
                CleanTransactionLocksList();
            }

            //if(c % PRIMENODES_DUMP_SECONDS == 0) DumpPrimenodes();
