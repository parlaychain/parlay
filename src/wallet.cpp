...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CParlayAddress(redeemScript.GetID()).ToString();
                           ...
                           list<pair<CTxDestination, int64_t> >& listSent, CAmount& nFee, string& strSentAccount, const isminefilter& filter) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // PARNOTE: CoinControl possible fix related... with HD wallet we need to report change?
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s PAR %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useIX) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (nDepth <= 0) // PARNOTE: coincontrol fix / ignore 0 confirm
                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !(fPrimeNode && pcoin->vout[i].nValue == GetMNCollateral(pindexBest->nHeight)*COIN);
                } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN){
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fPrimeNode) found = pcoin->vout[i].nValue != GetMNCollateral(pindexBest->nHeight)*COIN; // do not use Hot MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++){
                ...
                if (pcoin->vout[i].nValue == GetMNCollateral(pindexBest->nHeight)*COIN){
                    ...
                    //LogPrintf("CWallet::AvailableCoinsForStaking - Found Primenode collateral.\n");
                    found = true;
                    break;
                }
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1PAR
    if(coin_type == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1PAR over
                ){
                    CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                    int rounds = GetInputDarksendRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        // primenode-like input should not be selected by AvailableCoins now anyway
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            //
            // bit 0 - 1000PAR+1 ( bit on if present )
            // bit 1 - 100PAR+1
            // bit 2 - 10PAR+1
            // bit 3 - 1PAR+1
            // bit 4 - .1PAR+1
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fPrimeNode && out.tx->vout[out.i].nValue == GetMNCollateral(pindexBest->nHeight)*COIN) continue; //primenode input
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 1000 PAR.");
                    } else {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-Parlay-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        if (fDebug)
        {
            ...
            CParlayAddress coinAddress(keyID);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Primenode Payments
    int payments = 1;
    // start primenode payments
    bool bPrimeNodePayment = true; // note was false, set true to test

    if ( Params().NetworkID() == CChainParams::TESTNET ){
        if (GetTime() > START_PRIMENODE_PAYMENTS_TESTNET ){
            bPrimeNodePayment = true;
        }
    }else{
        if (GetTime() > START_PRIMENODE_PAYMENTS){
            bPrimeNodePayment = true;
        }
    }
    ...
    if(bPrimeNodePayment) {
        //spork
        if(!primenodePayments.GetBlockPayee(pindexPrev->nHeight+1, payee, vin)){
            CPrimenode* winningNode = mnodeman.GetCurrentPrimeNode(1);
            if(winningNode){
                payee = GetScriptForDestination(winningNode->pubkey.GetID());
                payeerewardaddress = winningNode->rewardAddress;
                payeerewardpercent = winningNode->rewardPercentage;
            } else {
                return error("CreateCoinStake: Failed to detect primenode to pay\n");
            }
        }
    }
    // If reward percent is 0 then send all to primenode address
    if(hasPayment && payeerewardpercent == 0){
        ...
        LogPrintf("Primenode payment to %s\n", address2.ToString().c_str());
    }
    ...
    int64_t primenodePayment = GetPrimenodePayment(pindexPrev->nHeight+1, nReward);

    // Set output amount
    if(hasPayment && txNew.vout.size() == 4 && (payeerewardpercent == 0 || payeerewardpercent == 100)) // 2 stake outputs, stake was split, plus a primenode payment, no reward split
    {
        txNew.vout[payments-1].nValue = primenodePayment;
        blockValue -= primenodePayment;
        txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
        txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
    }
    else if(hasPayment && txNew.vout.size() == 3 && (payeerewardpercent == 0 || payeerewardpercent == 100)) // only 1 stake output, was not split, plus a primenode payment, no reward split
    {
        txNew.vout[payments-1].nValue = primenodePayment;
        blockValue -= primenodePayment;
        txNew.vout[1].nValue = blockValue;
    }
    else if(hasPayment && txNew.vout.size() == 5 && payeerewardpercent > 0 && payeerewardpercent < 100) // 2 stake outputs, stake was split, plus a primenode payment
    {
        txNew.vout[payments-2].nValue = (primenodePayment / 100) * (100 - payeerewardpercent);
        txNew.vout[payments-1].nValue = primenodePayment - txNew.vout[payments-2].nValue;
        blockValue -= primenodePayment;
        txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
        txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
    }
    else if(hasPayment && txNew.vout.size() == 4 && payeerewardpercent > 0 && payeerewardpercent < 100) // only 1 stake output, was not split, plus a primenode payment
    {
        txNew.vout[payments-2].nValue = (primenodePayment / 100) * (100 - payeerewardpercent);
        txNew.vout[payments-1].nValue = primenodePayment - txNew.vout[payments-2].nValue;
        blockValue -= primenodePayment;
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CParlayAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CParlayAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CParlayAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s PAR %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s PAR %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
