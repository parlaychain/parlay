...
string strPrimeNodePrivKey = "";
string strPrimeNodeAddr = "";
bool fLiteMode = false;
bool fEnableInstantX = true;
int nInstantXDepth = 10;
int nDarksendRounds = 2;
int nAnonymizeParlayAmount = 1000;
int nLiquidityProvider = 0;
/** Spork enforcement enabled time */
int64_t enforcePrimenodePaymentsTime = 4085657524;
int nPrimenodeMinProtocol = 0;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Parlay
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Parlay
    // Mac: ~/Library/Application Support/Parlay
    // Unix: ~/.Parlay
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Parlay";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Parlay.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;
    return pathConfigFile;
}

boost::filesystem::path GetPrimenodeConfigFile()
{
    boost::filesystem::path pathConfigFile(GetArg("-mnconf", "primenode.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Parlayd.pid"));
