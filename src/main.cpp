...
const string strMessageMagic = "Parlay Signed Message:\n";
...
const int YEARLY_BLOCKCOUNT = 350400; //350400 block will execute with in  12 months
...
int64_t GetProofOfStakeReward(int nHeight, const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	if (nHeight < YEARLY_BLOCKCOUNT)						// 1st year
		...
		nSubsidy = STATIC_POS_REWARD * 0.9;
	else if (nHeight < (3 * YEARLY_BLOCKCOUNT))				// 3rd year
		nSubsidy = STATIC_POS_REWARD * 0.9 * 0.9;
	else if (nHeight < (4 * YEARLY_BLOCKCOUNT))				// 4th year
		nSubsidy = STATIC_POS_REWARD * 0.9 * 0.9 * 0.9;
	else if (nHeight < (5 * YEARLY_BLOCKCOUNT))				// 5th year
		nSubsidy = STATIC_POS_REWARD * 0.9 * 0.9 * 0.9 * 0.9;
	else 													// After 5th year
		nSubsidy = STATIC_POS_REWARD * 0.9 * 0.9 * 0.9 * 0.9 * 0.9;
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
	...
	bool PrimenodePayments = false;
	bool fIsInitialDownload = IsInitialBlockDownload();

	if (nTime > START_PRIMENODE_PAYMENTS) PrimenodePayments = true;
	if (!fIsInitialDownload)
	{
		if (PrimenodePayments)
		{
			...
			if (IsProofOfStake() && pindex != NULL) {
				if (pindex->GetBlockHash() == hashPrevBlock) {
					// If we don't already have its previous block, skip primenode payment step
					CAmount primenodePaymentAmount;
					for (int i = vtx[1].vout.size(); i-- > 0; ) {
						primenodePaymentAmount = vtx[1].vout[i].nValue;
						break;
					}
					...
					CScript payee;
					CTxIn vin;
					if (!primenodePayments.GetBlockPayee(pindexBest->nHeight + 1, payee, vin) || payee == CScript()) {
						foundPayee = true; //doesn't require a specific payee
						foundPaymentAmount = true;
						foundPaymentAndPayee = true;
						if (fDebug) { LogPrintf("CheckBlock() : Using non-specific primenode payments %d\n", pindexBest->nHeight + 1); }
					}

					for (unsigned int i = 0; i < vtx[1].vout.size(); i++) {
						if (vtx[1].vout[i].nValue == primenodePaymentAmount)
							foundPaymentAmount = true;
						if (vtx[1].vout[i].scriptPubKey == payee)
							foundPayee = true;
						if (vtx[1].vout[i].nValue == primenodePaymentAmount && vtx[1].vout[i].scriptPubKey == payee)
							foundPaymentAndPayee = true;
					}
					...
					if (!foundPaymentAndPayee) {
						if (fDebug) { LogPrintf("CheckBlock() : Couldn't find primenode payment(%d|%d) or payee(%d|%s) nHeight %d. \n", foundPaymentAmount, primenodePaymentAmount, foundPayee, address2.ToString().c_str(), pindexBest->nHeight + 1); }
						return DoS(100, error("CheckBlock() : Couldn't find primenode payment or payee"));
					}
					else {
					...
					if (fDebug) { LogPrintf("CheckBlock() : Skipping primenode payment check - nHeight %d Hash %s\n", pindexBest->nHeight + 1, GetHash().ToString().c_str()); }
				}
			}
			else {
				if (fDebug) { LogPrintf("CheckBlock() : pindex is null, skipping primenode payment check\n"); }
			}
		}
		else {
			if (fDebug) { LogPrintf("CheckBlock() : skipping primenode payment checks\n"); }
		}
	}
	else {
		if (fDebug) { LogPrintf("CheckBlock() : Is initial download, skipping primenode payment check %d\n", pindexBest->nHeight + 1); }
	}
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
	...
	if (!IsInitialBlockDownload()) {
		...
		// If we're in LiteMode disable darksend features without disabling primenodes
		if (!fLiteMode && !fImporting && !fReindex && pindexBest->nHeight > Checkpoints::GetTotalBlocksEstimate()) {

			if (primenodePayments.GetBlockPayee(pindexBest->nHeight, payee, vin)) {
				//UPDATE PRIMENODE LAST PAID TIME
				CPrimenode* pmn = mnodeman.Find(vin);
				if (pmn != NULL) {
					pmn->nLastPaid = GetAdjustedTime();
				}

				LogPrintf("ProcessBlock() : Update Primenode Last Paid Time - %d\n", pindexBest->nHeight);
			}

			darkSendPool.CheckTimeout();
			darkSendPool.NewBlock();
			primenodePayments.ProcessBlock(GetHeight() + 10);
...
bool static AlreadyHave(CTxDB& txdb, const CInv& inv)
{
	...
	case MSG_PRIMENODE_WINNER:
		return mapSeenPrimenodeVotes.count(inv.hash);
	}
	...
	while (it != pfrom->vRecvGetData.end()) {
		...
		const CInv &inv = *it;
		{
			...
			else if (inv.IsKnownType())
			{
				...
				if (!pushed && inv.type == MSG_PRIMENODE_WINNER) {
					if (mapSeenPrimenodeVotes.count(inv.hash)) {
						CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
						ss.reserve(1000);
						ss << mapSeenPrimenodeVotes[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
	...
	else if (strCommand == "tx" || strCommand == "dstx")
	{
		...
		if (strCommand == "tx") {
			...
			if (AlreadyHave(txdb, inv))
				return true;
			//these allow primenodes to publish a limited amount of free transactions

			CPrimenode* pmn = mnodeman.Find(vin);
			if (pmn != NULL)
			{
				if (!pmn->allowFreeTx) {
					//multiple peers can send us a valid primenode transaction
					if (fDebug) LogPrintf("dstx: Primenode sending too many transactions %s\n", tx.GetHash().ToString().c_str());
					return true;
				}
				...
				if (!darkSendSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage, errorMessage)) {
					LogPrintf("dstx: Got bad primenode address signature %s \n", vin.ToString().c_str());
					//Misbehaving(pfrom->GetId(), 20);
					return false;
				}

				LogPrintf("dstx: Got Primenode transaction %s\n", tx.GetHash().ToString().c_str());
	...
	else
	{
		...
		ProcessMessagePrimenodePayments(pfrom, strCommand, vRecv);
...
int64_t GetPrimenodePayment(int nHeight, int64_t blockValue)
{
