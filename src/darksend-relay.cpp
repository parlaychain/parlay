...
CDarkSendRelay::CDarkSendRelay(CTxIn& vinPrimenodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinPrimenode = vinPrimenodeIn;
    vchSig = vchSigIn;
...
std::string CDarkSendRelay::ToString()
{
    ...
    info << "vin: " << vinPrimenode.ToString() <<
...
{
    CPrimenode* pmn = mnodeman.GetPrimenodeByRank(nRank, nBlockHeight, MIN_POOL_PEER_PROTO_VERSION);
