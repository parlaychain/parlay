...
//step 2.) Top 10 primenodes, open connect to top 1 primenode. Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 primenode, waits for 10 messages. Upon success, sends "txlock'
...
void ProcessMessageInstantX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "txlvote") //InstantX Lock Consensus Votes
    {
        ...
        if(ProcessConsensusVote(pfrom, ctx)){
            //Spam/Dos protection
            /*
                Primenodes will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if(!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)){
                if(!mapUnknownVotes.count(ctx.vinPrimenode.prevout.hash)){
                    mapUnknownVotes[ctx.vinPrimenode.prevout.hash] = GetTime()+(60*10);
                }

                if(mapUnknownVotes[ctx.vinPrimenode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinPrimenode.prevout.hash] - GetAverageVoteTime() > 60*10){
                        LogPrintf("ProcessMessageInstantX::txlreq - primenode is spamming transaction votes: %s %s\n",
                            ctx.vinPrimenode.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which primenodes
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetPrimenodeRank(activePrimenode.vin, nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

    if(n == -1)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Unknown Primenode\n");
        return;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Primenode not in the top %d (%d)\n", INSTANTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinPrimenode = activePrimenode.vin;
...
{
    int n = mnodeman.GetPrimenodeRank(ctx.vinPrimenode, ctx.nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

    CPrimenode* pmn = mnodeman.Find(ctx.vinPrimenode);
    if(pmn != NULL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Primenode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);
    }

    if(n == -1)
    {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Unknown Primenode\n");
        mnodeman.AskForMN(pnode, ctx.vinPrimenode);
        return false;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Primenode not in the top %d (%d) - %s\n", INSTANTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if(!ctx.SignatureValid()) {
        LogPrintf("InstantX::ProcessConsensusVote - Signature invalid\n");
        //don't ban, it could just be a non-synced primenode
        mnodeman.AskForMN(pnode, ctx.vinPrimenode);
        return false;
    }
...
{
    return vinPrimenode.prevout.hash + vinPrimenode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    CPrimenode* pmn = mnodeman.Find(vinPrimenode);

    if(pmn == NULL)
    {
        LogPrintf("InstantX::CConsensusVote::SignatureValid() - Unknown Primenode\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(pmn->pubkey2, vchPrimeNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strPrimeNodePrivKey.c_str());

    if(!darkSendSigner.SetKey(strPrimeNodePrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid primenodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if(!darkSendSigner.SignMessage(strMessage, errorMessage, vchPrimeNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(pubkey2, vchPrimeNodeSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    ...
    {
        int n = mnodeman.GetPrimenodeRank(vote.vinPrimenode, vote.nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

        if(n == -1)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Primenode\n");
            return false;
        }

        if(n > INSTANTX_SIGNATURES_TOTAL)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Primenode not in the top %s\n", INSTANTX_SIGNATURES_TOTAL);
            return false;
        }
