...
{
    if(!fLiteMode && !fPrimeNode) disconnect(timer, SIGNAL(timeout()), this, SLOT(darkSendStatus()));
    delete ui;
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeParlayAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeParlayAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeParlayAmount = strAnonymizeParlayAmount.remove(strAnonymizeParlayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeParlayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeParlayAmount*COIN) nMaxToAnonymize = nAnonymizeParlayAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeParlayAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeParlayAmount));
        strAnonymizeParlayAmount = strAnonymizeParlayAmount.remove(strAnonymizeParlayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeParlayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
