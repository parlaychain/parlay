...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeParlayAmount"))
        settings.setValue("nAnonymizeParlayAmount", 1000);
    nAnonymizeParlayAmount = settings.value("nAnonymizeParlayAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeParlayAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeParlayAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeParlayAmount:
            return QVariant(nAnonymizeParlayAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeParlayAmount:
            nAnonymizeParlayAmount = value.toInt();
            settings.setValue("nAnonymizeParlayAmount", nAnonymizeParlayAmount);
            emit AnonymizeParlayAmountChanged(nAnonymizeParlayAmount);
            break;
        default:
            break;
        }
