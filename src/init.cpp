...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Parlay.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Parlayd.pid)") + "\n";
...
strUsage += "\n" + _("Primenode options:") + "\n";
    strUsage += "  -primenode=<n>            " + _("Enable the client to act as a primenode (0-1, default: 0)") + "\n";
    strUsage += "  -mnconf=<file>             " + _("Specify primenode configuration file (default: primenode.conf)") + "\n";
    strUsage += "  -mnconflock=<n>            " + _("Lock primenodes from primenode configuration file (default: 1)") + "\n";
    strUsage += "  -primenodeprivkey=<n>     " + _("Set the primenode private key") + "\n";
    strUsage += "  -primenodeaddr=<n>        " + _("Set external address:port to get to this primenode (example: address:port)") + "\n";
    strUsage += "  -primenodeminprotocol=<n> " + _("Ignore primenodes less than version (example: 61401; default : 0)") + "\n";

    strUsage += "\n" + _("Darksend options:") + "\n";
    strUsage += "  -enabledarksend=<n>          " + _("Enable use of automated darksend for funds stored in this wallet (0-1, default: 0)") + "\n";
    strUsage += "  -darksendrounds=<n>          " + _("Use N separate primenodes to anonymize funds  (2-8, default: 2)") + "\n";
    strUsage += "  -anonymizeParlayamount=<n> " + _("Keep N Parlay anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Process primenode config
    primenodeConfig.read(GetPrimenodeConfigFile());
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Parlay is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Parlay is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Parlay version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (mapArgs.count("-primenodepaymentskey")) // primenode payments priv key
    {
        if (!primenodePayments.SetPrivKey(GetArg("-primenodepaymentskey", "")))
            return InitError(_("Unable to sign primenode payment winner, wrong key?"));
        if (!sporkManager.SetPrivKey(GetArg("-primenodepaymentskey", "")))
            return InitError(_("Unable to sign spork message, wrong key?"));
    }

    //ignore primenodes below protocol version
    nPrimenodeMinProtocol = GetArg("-primenodeminprotocol", MIN_POOL_PEER_PROTO_VERSION);

    if (fDaemon)
        fprintf(stdout, "Parlay server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Parlay") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Parlay to complete") << "\n";
    ...
    uiInterface.InitMessage(_("Loading primenode cache..."));

    CPrimenodeDB mndb;
    CPrimenodeDB::ReadResult readResult = mndb.Read(mnodeman);
    if (readResult == CPrimenodeDB::FileError)
        LogPrintf("Missing primenode cache file - mncache.dat, will try to recreate\n");
    ...
    if(fPrimeNode) {
        ...
        strPrimeNodeAddr = GetArg("-primenodeaddr", "");

        LogPrintf(" addr %s\n", strPrimeNodeAddr.c_str());

        if(!strPrimeNodeAddr.empty()){
            CService addrTest = CService(strPrimeNodeAddr, fNameLookup);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -primenodeaddr address: " + strPrimeNodeAddr);
            }
        }

        strPrimeNodePrivKey = GetArg("-primenodeprivkey", "");
        if(!strPrimeNodePrivKey.empty()){
            std::string errorMessage;

            CKey key;
            CPubKey pubkey;

            if(!darkSendSigner.SetKey(strPrimeNodePrivKey, errorMessage, key, pubkey))
            {
                return InitError(_("Invalid primenodeprivkey. Please see documenation."));
            }

            activePrimenode.pubKeyPrimenode = pubkey;

        } else {
            return InitError(_("You must specify a primenodeprivkey in the configuration. Please see documentation for help."));
        }
    ...
    nAnonymizeParlayAmount = GetArg("-anonymizeParlayamount", 0);
    if(nAnonymizeParlayAmount > 999999) nAnonymizeParlayAmount = 999999;
    if(nAnonymizeParlayAmount < 2) nAnonymizeParlayAmount = 2;
    ...
    //lite mode disables all Primenode and Darksend related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if(fPrimeNode && fLiteMode){
        return InitError("You can not start a primenode in litemode");
    }

    LogPrintf("fLiteMode %d\n", fLiteMode);
    LogPrintf("nInstantXDepth %d\n", nInstantXDepth);
    LogPrintf("Darksend rounds %d\n", nDarksendRounds);
    LogPrintf("Anonymize Parlay Amount %d\n", nAnonymizeParlayAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1PAR+1000 == (.1PAR+100)*10
       10PAR+10000 == (1PAR+1000)*10
    */
