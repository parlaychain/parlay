...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Parlay is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Parlay is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Parlay is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Parlay is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "        ,...\n"
            "  ],\n"
            "  \"primenode_payments\" : true|false,         (boolean) true, if primenode payments are enabled"
            "  \"enforce_primenode_payments\" : true|false  (boolean) true, if primenode payments are enforced"
