...
{
    // Look for an existing connection. If found then just add it to primenode list.
    ...
    {
        LogPrint("net", "connected primenode %s\n", addrConnect.ToString());
        closesocket(hSocket);
...
*/        
    ...
    LogPrint("net", "connecting to primenode %s failed\n", addrConnect.ToString());
    return false;
}
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Parlay " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Parlay is probably already running."), addrBind.ToString());
        else
