...
namespace {
    class CParlayAddressVisitor : public boost::static_visitor<bool> {
    private:
        CParlayAddress *addr;
    public:
        CParlayAddressVisitor(CParlayAddress *addrIn) : addr(addrIn) { }
...
bool CParlayAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CParlayAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CParlayAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CParlayAddressVisitor(this), dest);
}

bool CParlayAddress::IsValid() const {
...
CTxDestination CParlayAddress::Get() const {
    if (!IsValid())
...
bool CParlayAddress::GetKeyID(CKeyID &keyID) const {
...
bool CParlayAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CParlaySecret::SetKey(const CKey& vchSecret) {
...
CKey CParlaySecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CParlaySecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CParlaySecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CParlaySecret::SetString(const std::string& strSecret) {
