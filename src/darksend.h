...
class CPrimeNodeVote;
class CParlayAddress;
...
#define POOL_STATUS_FINALIZE_TRANSACTION       4 // prime node will broadcast what it accepted
...
#define PRIMENODE_ACCEPTED                    1
#define PRIMENODE_REJECTED                    0
#define PRIMENODE_RESET                       -1
...
extern std::string strPrimeNodePrivKey;
extern map<uint256, CDarksendBroadcastTx> mapDarksendBroadcastTxes;
extern CActivePrimenode activePrimenode;
...
public:
     ...
     *     1) we have an active Primenode,
     *     2) we have a valid Primenode private key,
    ...
    /// Check if we have a valid Primenode address
    bool CheckSignature();
...
{
public:
    /// Is the inputs associated with this public key? (and there is 500 PAR - checking if valid primenode)
...
private:
    ...
    std::vector<CDarkSendEntry> entries; // Primenode entries
    ...
    bool sessionFoundPrimenode; //If we've found a compatible Primenode
...
public:
    ...
    CPrimenode* pSubmittedToPrimenode;
    ...
    {
        if (fPrimeNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)){
            LogPrint("darksend", "CDarksendPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Primenode. \n");
            return;
        }
    ...
    /// As a client, send a transaction to a Primenode to start the denomination process
    void SendDarksendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    /// Get Primenode updates about the progress of Darksend
