...
CCriticalSection cs_primenodes;
// keep track of the scanning errors I've seen
map<uint256, int> mapSeenPrimenodeScanningErrors;
...
CPrimenode::CPrimenode(const CPrimenode& other)
{
    LOCK(cs);
    vin = other.vin;
    addr = other.addr;
...
CPrimenode::CPrimenode(CService newAddr, CTxIn newVin, CPubKey newPubkey, std::vector<unsigned char> newSig, int64_t newSigTime, CPubKey newPubkey2, int protocolVersionIn, CScript newRewardAddress, int newRewardPercentage)
{
    LOCK(cs);
    vin = newVin;
    addr = newAddr;
...
//
// Deterministically calculate a given "score" for a primenode depending on how close it's hash is to
// the proof of work for that block. The further away they are the better, the furthest will win the election
// and get paid this block
//
uint256 CPrimenode::CalculateScore(int mod, int64_t nBlockHeight)
{
...
void CPrimenode::Check()
{
    ...
    if(activeState == PRIMENODE_VIN_SPENT) return;


    if(!UpdatedWithin(PRIMENODE_REMOVAL_SECONDS)){
        activeState = PRIMENODE_REMOVE;
        return;
    }

    if(!UpdatedWithin(PRIMENODE_EXPIRATION_SECONDS)){
        activeState = PRIMENODE_EXPIRED;
        return;
    }
	...
	if(!AcceptableInputs(mempool, tx, false, NULL)){
            activeState = PRIMENODE_VIN_SPENT;
            return;
        }
    }

    activeState = PRIMENODE_ENABLED; // OK
}